...
class CPCModel(nn.Module):
    def __init__(self, repr_dim=320, k_steps=3):
        ...
        self.predictors = nn.ModuleList(
            [nn.Linear(repr_dim, repr_dim) for _ in range(k_steps)]
        )
...
def train_cpc(
    ...
    model = CPCModel(repr_dim=repr_dim, k_steps=k_steps).to(device)
...
def main():
    ...
    figure_dir = PROJECT_ROOT / "outputs" / "figures" / "cpc"
    repr_dir = PROJECT_ROOT / "outputs" / "representations" / "cpc"
    checkpoint_dir = PROJECT_ROOT / "outputs" / "checkpoints" / "cpc"
    ...
    checkpoint_path = checkpoint_dir / "cpc_encoder.pt"
    ...
    rank_path = figure_dir / "cpc_linear_probe_rank.png"
    ranks_path = repr_dir / "cpc_linear_probe_ranks.npy"
