...
def random_shift_1d(x, max_shift=10):
    # x: [B, L, 1]
    if max_shift <= 0:
        return x
    ...
    for i, shift in enumerate(shifts):
        out[i] = torch.roll(x[i], shifts=int(shift.item()), dims=0)
    return out


def random_time_mask_1d(x, mask_ratio=0.05):
    # x: [B, L, 1]
    if mask_ratio <= 0:
        return x

    out = x.clone()
    batch_size, trace_len, _ = out.shape
    mask_len = max(1, int(round(trace_len * mask_ratio)))

    if mask_len >= trace_len:
        out.zero_()
        return out

    starts = torch.randint(
        low=0,
        high=trace_len - mask_len + 1,
        size=(batch_size,),
        device=out.device,
    )
...
def augment_trace(
    ...
    y = random_shift_1d(x, max_shift=max_shift)

    if scale_std > 0:
        scale = 1.0 + scale_std * torch.randn(
            y.size(0),
            1,
            1,
            device=y.device,
    ...
    y = random_time_mask_1d(y, mask_ratio=mask_ratio)
    return y


class ConvTraceEncoder(nn.Module):
    """
    Conv1D backbone for BYOL on ASCAD traces.
...
class BYOL1D(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        loss = 0.5 * (
            self.byol_loss(pred1, target_z2)
            + self.byol_loss(pred2, target_z1)
        )
...
def train_byol(
    ...
    for epoch in range(n_epochs):
        ...
        print(f"Epoch #{epoch}: byol_loss={avg_loss:.6f}")
...
def main():
    ...
    print("Training linear classifier on BYOL representations...")
    ...
    plot_rank_curve(
        ranks,
        save_path=rank_path,
        title="BYOL-1D + Linear Probe Key Rank",
    )
