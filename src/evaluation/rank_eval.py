from tqdm import tqdm
from pathlib import Path
from typing import Optional, Sequence, Union

import matplotlib.pyplot as plt
import numpy as np


AES_SBOX = np.array([
    
], dtype=np.uint8)


def expand_proba_to_256(
    probas: np.ndarray,
    classes: Optional[Sequence[int]] = None,
    eps: float = 1e-40,
) -> np.ndarray:
    
    probas = np.asarray(probas, dtype=np.float64)

    if classes is None:
        if probas.shape[1] != 256:
            raise ValueError(f"Expected probas with 256 columns, got {probas.shape}")
        full = probas
    else:
        classes = np.asarray(classes, dtype=np.int64)

        full = np.full((probas.shape[0], 256), eps, dtype=np.float64)
        full[:, classes] = probas

    # Re-normalize to make every row sum to 1.
    row_sum = full.sum(axis=1, keepdims=True)
    row_sum = np.where(row_sum == 0, 1.0, row_sum)
    full = full / row_sum

    return full


def compute_rank_curve(
    probas: np.ndarray,
    metadata: np.ndarray,
    target_byte: int = 2,
    max_traces: Optional[int] = None,
    use_log: bool = True,
    eps: float = 1e-40,
) -> np.ndarray:
    
    probas = np.asarray(probas, dtype=np.float64)

    if probas.ndim != 2 or probas.shape[1] != 256:
        raise ValueError(f"probas must have shape (N, 256), got {probas.shape}")

    num_traces = probas.shape[0]
    if max_traces is not None:
        num_traces = min(num_traces, max_traces)

    if len(metadata) < num_traces:
        raise ValueError("metadata length is smaller than number of traces used.")

    true_key = int(metadata[0]["key"][target_byte])
    key_candidates = np.arange(256, dtype=np.uint8)

    scores = np.zeros(256, dtype=np.float64)
    ranks = np.zeros(num_traces, dtype=np.int64)

    for i in tqdm(range(num_traces), desc="Computing key rank"):
        plaintext_byte = int(metadata[i]["plaintext"][target_byte])

        candidate_labels = AES_SBOX[np.bitwise_xor(plaintext_byte, key_candidates)]

        key_probs = probas[i, candidate_labels]

        if use_log:
            scores += np.log(key_probs + eps)
        else:
            scores += key_probs

        ranks[i] = int(np.sum(scores > scores[true_key]))

    return ranks


def plot_rank_curve(
    ranks: np.ndarray,
    save_path: Union[str, Path],
    title: str = "Key Rank Curve",
) -> None:

    save_path = Path(save_path)
    save_path.parent.mkdir(parents=True, exist_ok=True)

    plt.figure()
    plt.plot(np.arange(1, len(ranks) + 1), ranks)
    plt.xlabel("Number of attack traces")
    plt.ylabel("Rank")
    plt.title(title)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(save_path)
    plt.close()